            ...
            switch (settings.shape) {
                ...
                case 'rectangle':
                    ...
                    settings.shape = function shapeRectangle(theta) {
                        ...
                        var thetaPrime = theta % (2 * Math.PI / 4);
                        var r = 1 / (Math.cos(thetaPrime) + aspectRatio * Math.sin(thetaPrime));
                        if (Math.abs(Math.cos(thetaPrime)) > Math.abs(Math.sin(thetaPrime))) {
                            r = aspectRatio / Math.abs(Math.sin(thetaPrime));
                        } else {
                            r = 1 / Math.abs(Math.cos(thetaPrime));
                        }
                        return r;
                    }
                    break;
